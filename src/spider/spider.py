...
class Spider:
    ...
    def __init__(self):
        ...
        aws_session = boto3.session.Session(
            aws_access_key_id=self.config.get("s3").get("key"),
            aws_secret_access_key=self.config.get("s3").get("secret"),
        )
        self.s3 = aws_session.client(service_name="s3")
    ...
    def _anti_hammer_sleep(self):
        ...
        sleep(randrange(1, self.config["anti_hammer_sleep"]))  # nosec
    ...
    def crawl(self):
        ...
        self.feed.link(
            href=self.config.get("s3")
            .get("object_url")
            .format(
                bucket=self.config.get("s3").get("bucket"),
                region=self.config.get("s3").get("region"),
                filekey=self.config.get("s3").get(f"feed_filename_{self.environment}"),
            ),
            rel="self",
        )
    ...
    def _upload_feed(self):
        ...
        key = (
            self.config.get("s3")
            .get(f"feed_filename_{self.environment}")
            .format(version=getenv("FEED_VERSION", "v0"))
        )
        self.s3.upload_fileobj(BytesIO(atomfeed), bucket, key)
        resp = self.s3.put_object_acl(ACL="public-read", Bucket=bucket, Key=key)
        if resp is None:
            logging.warning(f"Failed to set object ACL for {bucket}/{key}")
    ...
    def _get(self, url, **kwargs) -> Response:
        ...
        if resp.status_code in self.TIMEOUT_STATUS_CODES:
    ...
    @retry(TimeOutException, tries=5, delay=3, backoff=2)
    def _login(self):
        login_url = self.config.get("site").get("login_url")
    ...
    def _cover_image_upload_and_get_url(self, url) -> str:
        ...
        if matches is None:
            logging.warning(f"no cover image found on page {url}")
            return ""
    ...
    def _thumbnail_small_image_upload_and_get_urls(self, urls) -> List:
        ...
        for url in urls:
            ...
            if matches is None:
                logging.warning(f"no small images found on page {url}")
                continue
    ...
    def _thumbnail_large_image_upload_and_get_urls(self, urls) -> List:
        ...
        for url in urls:
            ...
            if matches is None:
                logging.warning(f"no large images found on page {url}")
                continue
